...
public class BrowseMoviesGUI extends JFrame{
	...
	private JButton returnHomeButton; //need to add
	private JLabel lblNewLabel; // need to add
	...
	public BrowseMoviesGUI(String[][] moviesWithAccess, boolean userStatus, ShowTime[] showtimes) {
		...
		getContentPane().add(searchByShowtimeLabel);
		...
		getContentPane().add(confirmSelectionButton);
		
		posterLabel = new JLabel();
		posterLabel.setBounds(102, 335, 226, 265);
		getContentPane().add(posterLabel);
        ...
        lblNewLabel = new JLabel("Login Status: Not Logged In");
        lblNewLabel.setBackground(new Color(255, 222, 173));
        lblNewLabel.setBounds(298, 8, 160, 35);
        getContentPane().add(lblNewLabel);
        
        if(userStatus == true) {
        	lblNewLabel.setText("Login Status: Logged In");
        }
	...
	private void setUpTable(String [][] moviesWithAccess) {
		table = new JTable() {
			private static final long serialVersionUID = 1L;
	...
	public void setTableModel(String [][] moviesWithAccess) {
		table.setModel(new DefaultTableModel (moviesWithAccess, headers));
	}
